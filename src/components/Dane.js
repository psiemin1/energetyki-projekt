...
const sectionTitles = {
  "Częstotliwość": 'Częstotliwość spożywania napojów energetycznych',
  "Powody": 'Powody sięgania po napoje energetyczne',
  "Sytuacje": 'Sytuacje, w których uczniowie sięgają po napoje energetyczne',
  "Reakcje": 'Reakcje fizyczne po spożyciu',
...
function Dane() {
  ...
  return (
    <div className="sticky-charts-container" style={{ padding: '5px 20px' }}>
...
const ScrollRevealChart = ({ data, kategoriaKey, title, sectionColors, selectedKeys }) => {
  ...
  const calculateBarSize = () => {
    const baseSize = 300;
    const scalingFactor = Math.max(1, selectedKeys.length * 2);
    return baseSize / scalingFactor;
  };

  return (
    <div ref={ref} className={`chart-container fade-in ${inView ? 'in-view' : ''}`} style={{ marginTop: '-20px' }}>
      <h2>{title}</h2>
